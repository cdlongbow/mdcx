...
- Instructions:
  ...
  - **Qt 同名 API 重载签名不同，改前确认目标类签名**；测试桩显式枚举属性方法（不用 __getattr__ 通配）；打包前逐页切 stackedWidget 审计边界溢出（scripts/check_ui_layout.py、tests/test_ui_geometry.py）。
   ...
   - **带浮框/遮挡底的滚动区底部余量必须 > 浮框侵入量**（2026-09-06 实证）：浮框带侵入视口底部 63px，内容 `_CONTENT_BOTTOM_MARGIN = 60` 仅差 3px 就把最后一行头顶进了带底；layout 驱动内容（NFO）的 min_height 用 `layout.sizeHint().height()` 还没加余量，末行直接贴视口底。纪律：滚到底时「最后一行底 = 视口底 - margin」，**margin 必须 ≥ 浮框侵入 + 视觉缓冲**（当前 72 > 63）；layout 驱动分支的 min_height = sizeHint.height() + margin，与非 layout 分支（childrenRect.bottom + margin）一致。既有测试 `test_setting_content_clears_config_bar_when_scrolled` 锁定。
