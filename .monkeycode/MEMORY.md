...
- Instructions:
  ...
  - **运行库现状**（2026-09-01 治理收官）：主表 26580 行（零错挂零重复）+ 待修正 923 + DMM已覆盖归档 14458 + 四源校验错配 ~20（人工待验）。**出厂库（resources/userdata/，9700 行旧数据）更新须用户明确确认**。证据产物在 /tmp/opencode/（verify_result/libre/jb/app_title_recheck/reverse_result/fix_map.json），备份链 asin_db_backup_*。
