...
- **最大化后内容自适应窗口（第二轮，覆盖软件界面/软件工具/软件设置全量内容）**：①软件工具/设置页 groupBox 拉宽后内部表单仍缩在左侧——输入类控件（输入框/下拉/列表/树）与布局容器现随容器拉宽（布局容器显式 invalidate+activate 强制重排，Qt 对休眠页布局不自动激活是隐藏根因），右缘按钮（浏览/选择）右缘锚定平移，左侧标签保持原位；②软件界面页信息区（简介/标签/标题/番号/演员）拉伸贴结果树列，上半区右列（系列/发行商等）整组平移，上区行按同行按钮限制锚定右界；③设置页底部「当前配置/另存为/恢复默认/保存」操作浮框贴新底部、保存按钮右缘锚定、背景拉伸贴宽（此前悬在半空）。全部同步用「设计基准+extra」固定公式，缩放往返幂等无漂移
