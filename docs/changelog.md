...
- **议题 #72 隐藏入口后导航间隙不齐 + 导航改名**：#71 的隐藏开关只 `setVisible` 按钮，而导航 8 个按钮之间靠 7 个 8px 固定 `QSpacerItem` 分隔——spacer 是独立布局项，`setVisible(False)` 隐藏不掉，隐藏「演员管理」「信息管理」后相邻 spacer 残留叠出 16px 空洞且与其他 8px 间距不一致（#71 条目中"无需手动几何同步"的说法即漏检此处）。修复：删除全部 7 个按钮间固定 spacer，改用 layout spacing=8，Qt 对隐藏按钮自动收紧间距，隐藏后剩余按钮间距立即恢复一致；顺带按报告人建议将导航「Emby演员管理」→「演员管理」、「NFO库管理」→「信息管理」（设置页复选框「隐藏 Emby 演员管理/隐藏 NFO 库管理」为功能开关名称，保持不变），文档同步更新。`test_ui_structure.py` 新增回归锁定导航 spacing=8 且布局内不得再出现 spacer
- **议题 #73 启动自检范围说明**：报告人看到启动日志在 JavDb 返回「IP 被禁止」后无后续输出，误以为"网络检测出异常就直接停止检测"——实际该日志是启动自检（数据库 / ThePornDB / JavDb / JavBus 四项固定检查，各自独立线程全部执行完毕），全量站点检测在「检测网络」页，二者是独立链路，逻辑本身无缺陷。在自检开始处输出一行范围说明与全量检测入口指引，消除同类误读
- **议题 #71 左侧导航入口可隐藏**：同一报告人在 #61 曾要求删除 Emby 演员管理与 NFO 库管理（当时因底层被主刮削链引用且有活跃使用者而保留功能），现按其退让诉求在「设置 → 高级」新增「隐藏入口」开关——勾选后左侧导航对应入口即刻隐藏（导航为 QVBoxLayout 自动收紧，无需手动几何同步），复选框取消即恢复，保存后立即生效；仅隐藏入口、功能本身完整保留。新增 `Switch.HIDE_ACTOR_NAV/HIDE_NFO_NAV` 枚举并接入 save/load 双向绑定，`test_window_state_matrix.py` 新增用例锁定开关 → `setVisible` 与复选框回写
- **议题 #69 恢复系统最大化按钮 + 切换配置故障机制说明**：①恢复主窗口系统最大化按钮（撤销 #67 的 `WindowMaximizeButtonHint` 屏蔽）——#62/#66/#68 的最大化布局错乱根因（Qt 绝对定位缩放三连：`setGeometry` 不触发子组件 resizeEvent、QStackedWidget 只 resize 当前可见页、show_hide_logs 硬编码覆盖动态同步）当时已修复，`resizeEvent → _sync_page_layouts` 统一同步路径对最大化生效，禁用按钮只是绕过症状且与保留的拖拽边缘缩放能力自相矛盾；新增 `test_window_state_matrix.py` 用例锁定最大化按钮存在及 `showMaximized` 后全页面布局同步。②「不能切换配置」根因修复（报告人补充日志后定位）：含旧配置键 `amazon_strict_pic_verify` 的配置迁移时仅产生一条警告（「已移除配置项 严格校验 Amazon 图片」），但 `Config.update()` 返回的迁移警告未经标记混入了 `load_config` 的错误判定——任何非空消息都触发 `_failed.json` 保护分支，含该旧键的配置被误判为「读取配置文件出错」，每次切回都重新弹回且写入标记文件持久化，表现为「不能切换配置」。修复：迁移警告统一加 `[迁移]` 前缀（与 `[V1]` 通知同级），`load_config` 只对无前缀的真校验错误触发保护分支；`test_issue69_migration_notice.py` 三项锁定（警告必带前缀 / 迁移通知不改指 `_failed.json` / 真校验错误仍触发保护）
- **议题 #67 检测网络页按钮悬浮遮挡日志 + 隐藏最大化按钮**（最大化按钮后由议题 #69 决议恢复，见上条）：①`page_net` 文本区从 y=60 开始（原占满整页 y=0，「打开网络设置」等按钮 y=13 直接叠在日志首行上，getchu/javdb_api 的错误信息被遮住）；②删除「打开网络设置」按钮（与设置页导航完全重复，报告人点名嫌丑），保留「开始检测/重试失败项/复制结果」，「重试失败项」补齐右缘动态锚定（原仅 check_net/net_copy 锚定，缩放后按钮散开）；③检测完成建议文案同步改为「设置 → 网络」页指引；④主窗口隐藏系统最大化按钮（`WindowMaximizeButtonHint`）——#62/#66/#68 连续三个议题的布局错乱均由最大化触发，报告人要求直接禁用；拖拽边缘缩放与 `_sync_page_layouts` 同步逻辑保留
...
- **JavDB 全系图源无水印升级（App CDN 加密流解密）**：JavDB App 专用 CDN `tp.spfcas.com` 返回单字节 XOR 加密流（首字节为随机 key），解密后为无水印原图；网页版 CDN `c0.jdbstatic.com` 是同图带 javdb 水印版本。新增 `base/web.py::decrypt_spfcas_image`（XOR 解密 + JPEG SOI 有效性校验，`bytes.translate` 实现）与按域名判定的 `decode_spfcas_image_content` 统一解密入口，注入全部图片落盘路径（`MediaResourceContext._fetch_image` / `download_file_with_filepath` / `download_content_with_filepath`）——javdb_app 爬虫直接产出 spfcas URL 透明获得无水印封面/海报/剧照；javdb 与 javdb_api 的网页版 URL 由下载层前置 `jdbstatic_to_spfcas` 无水印变体自动升级（thumbs↔small_covers 路径映射），失败自然回退原图。App CDN 路径中段（`rhe951l4q`）可能随服务端调整变更：`learn_spfcas_image_segment` 由 javdb_app 每次响应学习自愈，中段过期时变体 404 自动回退网页版、行为退化为改动前；poster 选优（auto_best）模式下加密流尺寸探测失败用逆向网页版 URL 探测同图尺寸回退。解密失败（SOI 校验不过）按下载失败处理走 DMM 兜底链，不落坏图。新增 10 项回归测试（判定/解密/双向变换/中段学习/变体优先与回退/尺寸探测回退）

### 优化

- **DMM 直构静态路由种子 v2（libredmm 全站归纳 + 老片 append 兜底）**：刮削中发现的"libredmm 可反查真实 cid"路线固化为离线知识。libredmm.com `/movies` 列表页缩略图 src 自带完整 DMM 图床 cid，全站 23472 页 58.9 万「番号↔cid」对全量抓取后归纳为 `resources/userdata/dmm_cid_routes.json`（随包分发，1.8MB）：9267 个系列的 prefix→cid 构造规则 + 10530 条重编号白名单，`dmm_direct` 惰性加载——路由命中的系列（静态表需 `_COMMON_PREFIXES` 10 连盲试的冷门厂牌）首候选直达真实 cid，白名单特例番号精确返回唯一 cid，全链路零额外请求、彻底摆脱 libredmm 运行时代理依赖。安全过滤两轮演进：v1 发现 libredmm 同名番号被同名老厂牌占据（SSIS-001 在其记录中是 80 年代 mono 老厂牌 `k9ssis001` 而非 s1 的 `ssis00001`，IPX/SONE/MIDV/CAWD 等 8 系列直接注入会全部污染候选顺序）——但整体弃用静态表覆盖系列矫枉过正，把 GVG 等老片系列的真实 mono 3 位数据也丢了（日亚重裁决时 517 行裁判图 miss 暴露缺口，如 GVG-564 真实 cid 是 mono 的 `13gvg564`）；**v2 修正为 append 分级模式**：静态表覆盖系列的路由候选追加在静态表候选之后兜底（静态表保序首位、高频系列顺序零污染），未覆盖系列保持 first 一步直达，combo 按 digital 路径/5 位优先排序，老片番号静态表 5 位 miss 后由路由 3 位候选救回。`test_high_frequency_series_first_candidate_unchanged`/`test_append_mode_legacy_three_digit_fallback` 等 12 项回归测试锁定。`clean_asin_db_conflicts.py` 裁判链同步切换到生产直构器（废弃的 thejavdb API 链路清除）
- **javdb_app 签名失效 fail-fast 诊断与常量环境变量覆盖**：jdsignature 常量内嵌于 JavDB App APK，App 发新版可能轮换——届时三个 API 主机会同时返回 `InvalidSignature`(HTTP 400)/`ParameterInvalid`/401/403 类失败，原实现只记笼统的"API 请求失败"。现在统一响应包裹的错误 action（含 200 错误体）与 HTTP 状态码被归类为签名类失败：全部主机命中时直接给出明确诊断（签名常量可能已轮换、升级 mdcx 或设置环境变量），不再流入解析产出误导性的"未找到影片"；`MDCX_JAVDB_APP_SIG_PREFIX`/`MDCX_JAVDB_APP_SIG_SUFFIX`/`MDCX_JAVDB_APP_VERSION_NUMBER` 三个环境变量可免改码覆盖签名常量与版本号（连通性检测共用同一签名，同步生效）。搜索请求补 `type=movie` 过滤噪声与 `limit=50`（服务端上限，实测超限静默截断，默认偏小会漏精确命中）。机制文档沉淀 `docs/JAVDB_APP_SIGNATURE.md`
- **刮削失败原因 Top N 摘要**：大批量刮削结束后，失败明细前新增按类别聚合的摘要行（如 `📊 失败原因汇总（Top 3）：请求超时 23 个 ｜ 站点未收录 15 个 ｜ 番号未识别 8 个`），并根据头部类别给出一条可操作建议（超时→检查代理/调大重试、被拦截→换免 CF 通道、限流→降并发、未收录→调网站源顺序、番号未识别→整理命名、权限不足→管理员运行）。文本归一规则取自真实失败文案（`mdcx/core/fail_reason_stats.py`），用户不再需要逐条翻看几十条失败日志找主要矛盾
- **刮削进度预计剩余时间**：状态栏进度从「🔎 已刮削 320/5000」升级为「🔎 已刮削 320/5000 · 剩余约 2 小时 15 分」，大批量刮削时用户可感知进度节奏。基于最近 30 个完成事件的滑动窗口速率估算（少于 8 个样本不出估算，避免前期抖动给出误导数字），纯呈现层实现零流程风险（`mdcx/core/scrape_progress.py`）；完成回调对 estimator 的记录与文本附加由 AST 哨兵测试锁定
- **LLM 翻译同批次缓存**：系列片/合集片标题与简介高度相似，重刮失败列表时也会重复翻译完全相同的文本——LLM 按 token 计费，重复翻译纯浪费。新增进程内 LRU 翻译缓存（`mdcx/core/llm_translate_cache.py`，512 条）：相同 (prompt, 目标语言, 原文) 直接复用译文，**并发相同请求自动合并为一次调用**（in-flight Future 共享），翻译失败不缓存以便重试；缓存绑定 LLM 配置指纹（模型/接口地址/关闭思考开关），切换模型或服务商后自动失效重建。系列片批量刮削与失败列表重刮场景显著省 token 与时间
- **ASIN 数据库读零校验（信任快车道）**：ASIN 库随版本交付时番号↔ASIN↔封面图已经四源校验工程全量验证（图像双源 + 标题归一 + tenhow cid 结构化反查，主表 26580 行零错挂），刮削时库命中的番号直接采信下载日亚高清封面，**不再逐次软校验**——省去每片 1-2 次参考图下载与相似度比对。软校验保留唯一战场：搜索**新发现**的 ASIN 入库前必须通过图片相似度门槛，从源头拦截错配入库。连带移除「严格校验 Amazon 图片」开关（读零校验架构下语义为空：库命中无条件信任、新入库无条件必验），旧配置残留键由迁移静默清洗并提示
- **特典/限定版入库让位规则**（主表同番号重复治理）：一票否决清单拆分——**真合集词**（BEST/コンプリート/N時間/総集編等，图是多片拼盘）维持 v2 一票否决；**特典/限定版词**（メーカー特典/数量限定/生写真/初回限定，图仍是该单片真图）从否决清单移除——唯一 ASIN 时正常入库使用。同番号已有记录时按让位三态决策（`_decide_save_outcome`）：旧记录是特典版、新记录是正品 → 全行替换（`replace_asin_record`）；新特典或新旧同类 → 跳过先到先得。历史存量同步清理：IPX-315/IPZ-944/SSNI-089/SNIS-738/SSNI-623 五组"一正一特"重复行保留正品 ASIN，特典行移「待修正」可回滚
- **软校验裁决链 v2（新 ASIN 入库门槛升级）**：软匹配路径（标题/演员搜索发现）的入库裁决从单一图像比对升级为三步证据链——① **tenhow cid 旁证**（零请求本地查表：`resources/userdata/tenhow_asin_cids.json` 离线索引 36441 条映射，cid 反查番号一致即实锤、不一致强警示阻止）；② **标题门**（NFKC 归一系列互含——半角片假名/全角字母/括号特典统一，同系列不同集号/合集包装可判同片；**合集词一票否决**：日亚标题含 BEST/コンプリート/N時間 直接不入库，此为四源校验工程实证的错配最大病灶）；③ **图像门兜底**（无 cid 无标题证据时沿用现有相似度比对）。新模块 `mdcx/core/title_match.py`（标题归一算法产品化，303+34 行翻案零冤案实证）与 `mdcx/core/asin_cid_index.py`（367 行零误判实证）。**入库时序重构**：软匹配的写库从搜索阶段延迟到软校验通过后（`_save_asin_record` 从 soft 路径 4 处移除，统一在 web 采信点 `_save_verified_asin_record` 延迟入库）——v2 判失败的候选不再污染数据库；条码/番号硬匹配路径维持发现即入库（免验路径语义不变）。演员名兜底路径（错配重灾区）全链必走；主搜索路径的标题证据复用搜索阶段已聚合的商品标题（`amazon_match_state` 新增 title/search_keyword 携带），零额外请求
- **ASIN 数据库查询内存索引**：`query_asin_database` 原实现每次查询都全量解析 4 万行 xlsx（实测 2.2 秒/次），而刮削热路径每个影片都查一次 ASIN 缓存（`_check_asin_cache`）——刮 1000 片仅此一项就浪费 36 分钟纯 I/O。新增进程内双索引缓存（number/asin → 记录列表）：首次查询建索引，此后命中内存（实测 0.08ms，约 3 万倍加速）；文件 mtime 变化（写入/合并/工具清洗）自动失效重建，解析失败保留原有日志语义。演员/信息库的后台惰性加载（`start_data_loading`）此前已就位，本次补齐 ASIN 库最后一块
- **ASIN 出厂库扩容至 26620 行 + 合并升级出厂库权威覆盖**：出厂库经 2026-09-01~02 治理工程全量净化（四源校验、标题法双通道、tenhow cid 反查、人工裁决），从 9699 行扩容到 26620 行，同番号 ASIN 唯一、错配行清零。合并逻辑升级为"出厂库权威"（`merge_asin_db_from_backup`）：**同番号出厂库数据无条件覆盖用户库**番号外 5 列（纠正老用户历史错挂），用户库独有番号保留，出厂库独有番号追加。`_format_asin_worksheet` 新增按番号排序（前缀字母 + 数字升序），保存/合并/重排路径统一生效，出厂库与运行库打开即有序。测试断言反转并补"用户库独有番号保留"场景（`test_merge_asin_db_from_backup`）
- **DMM 高清直链升级推广到 11 个聚合/综合站爬虫**：`upgrade_dmm_cover`（横版 pl + 竖版 ps 双升级，自带无码跳过 + TTL 缓存 + 并发合并 + 失败回退原图）此前只接入 javbus/javdb/javdb_api/javdb_app/javlibrary/r18dev 六个主流爬虫，本次推广到 airav_cc/avbase/avsex/freejavbt/iqqtv/javday/javfree/lulubar/missav_api/thejavdb_api/xcity——其中 freejavbt/javday/iqqtv/lulubar 等爬虫原本 poster 为空，升级后首次获得竖版高清海报（poster 是 Emby/Jellyfin 显示质量关键字段；兜底链 `find_valid_dmm_cover` 只救 thumb 不写 poster，内嵌升级是唯一能给 poster 换高清的路径）；xcity 的 thumb=背面/poster=正面语义保持；avbase 保留其原有 pics→aws 域名升级链与之互补。新增 `test_dmm_upgrade_coverage.py` 4 项 AST 哨兵锁定 17 个爬虫的升级链防未来重构断链
...
- **全库审查 19 项缺陷修复（4 域并行审查 + 逐项独立复现验证）**：全部先写复现测试跑红再修；subagent 宣称 30 项经独立验证 19 项成立、3 项证伪（freejavbt remove 语义、iqqtv 双斜杠触发条件、curl_cffi 跨 loop），证伪项不修。**数据正确性（A 级 5 项）**：① v1 旧配置迁移链崩溃——ini 含 `javdb_website` 等自定义站点键时 `ConfigV1(**d)` 抛 TypeError，模块级 manager 实例化连锁崩溃整个应用且 MARK_FILE 已指向未生成文件导致旧配置永久丢失；`_website` 键改走 unknown_fields 通道由 `_update()` setattr 消化，`handle_v1` 整体加 try（失败回退默认配置+保留 v1 文件+提示排查）。② 无码官网番号前缀丢弃误路由——`1pondo-072625_101.mp4` 提取番号时剥掉前缀成 `072625_101`，official 爬虫按尾号长度猜站点误路由 pacopacomama（反向同理），拿到错误影片元数据当成功入库；带前缀形态保留前缀，路由按 `DIGIT_PREFIX_ALIASES` 精确判定，missav 判定链同步适配。③ `fill_missing_only` NFO 合并策略语义反转——实现与 `preserve_existing` 同分支（本地旧值覆盖新数据），与 UI 名称"仅填空字段"/docstring/models 描述三处矛盾，两选项行为完全重复；修正为新数据为主、空字段从本地补。④ 空番号共享缓存串味——指定 URL 重刮（番号留空）时 `""` 被注册为共享键，同批第二个此类文件直接复用第一个影片的标题/演员生成 NFO 张冠李戴；番号非空才启用共享。⑤ javbus/javlibrary FC2 搜索兜底匹配永不命中（站侧剥 PPV 番号侧没剥，归一不对称）；提取 `is_match` 纯函数并两侧对齐（经人工质疑验证降级：正常分类 FC2 走 website_fc2 组不经这两站，触发面限单站模式/指定 URL/自定义列表）。**可靠性（B 级 9 项）**：⑥ 流式请求 CF 挑战页判定失效（curl_cffi 流式 content 恒为空，has_marker 恒 False，分块下载遇 CF 403 只当普通错误重试耗尽）——判定改 async 经 `acontent()` 读体并缓存防重复读。⑦ av-wiki 演员真名查询空响应打穿整片刮削（`""` 穿透 `is None` 判定 → `fromstring("")` 返回 None → AttributeError，演员查询为非致命辅助步骤却让整片记失败）——`if not res` + 解析结果判 None 降级记日志。⑧ 失败原因被正常日志稀释——`error().get()` 聚合本组 log 通道几十行（字段来源/下载过程）随失败列表与 SQLite 持久化；`get()` 增 `only_self` 参数，error 消费点三处切换。⑨ extrafanart 部分失败静默吞（await 后不查返回值照常复制残缺目录到副本目录）——按返回值分支，False 时按 IGNORE_PIC_FAIL 语义计失败。⑩ trailer/thumb/poster/fanart 的 `file_done_dic` 锁外 check-then-act（同番号分集并发记录互相覆盖、复制兜底失效）——六处读检查全部移入 `_file_done_lock`。⑪ 保存设置裸 `int()`/`Website()` 转换无保护（清空输入框点保存抛 ValueError，异常点前已赋值的几十个字段残留内存呈半写状态）——`_int_or` 回退默认值 + 非法站点静默跳过对齐迁移层策略。⑫ 定时/间歇间隔 ≥24h 往返归零（`timedelta.seconds` 只含日内部分，48h 显示 00:00:00 再保存即丢失）——显示用 `total_seconds()`、输入正则放宽 3 位小时段。⑬ ASIN xlsx 五处保存全非原子（进程中断/磁盘满即 zip 半写损坏，下次启动合并异常被吞整个用户库静默废弃）——`_save_workbook_atomic`（tmp+os.replace+失败清理残留）；合并 marker 移到重排成功后写（防"数据已合并但永不重排"）；openpyxl 全表格式化（4 万行库秒级）挪 `asyncio.to_thread` 防阻塞事件循环；写路径补 `invalidate_asin_cache` 调用（原定义 0 调用点，失效全靠 mtime）。⑭ 断点续刮 failed 重试上限主路径失效（`should_skip` 只判 done，failed 文件在扫描范围内每次全量无限重试，MAX_RETRY_COUNT=3 形同虚设）+ 独立失败目录配置下跨会话恢复 100% 失效（`set_failed` 的 key 是移动后路径，`cleanup_missing` 按扫描集合判存活把 failed 记录全删）——主扫描过滤加 fail_count 超限跳过（区分超限与从未刮过）；`set_failed` 增 `origin_path` 列（ALTER TABLE 迁移），存活判定按 origin。**解析健壮性（C 级 5 项）**：⑮ getchu 搜索结果两个 XPath 列表按下标配对（a 内嵌 `<br>` 即错位/IndexError）→ 单次选元素配对取值；⑯ prestige 嵌套下标无守卫（API 结构漂移裸 KeyError 且不可诊断）→ `.get()` 链式取值；⑰ javday 封面用绝对位置 `meta[8]`（head 增删任一 meta 即静默取错）→ og:image 语义定位；⑱ r18dev cid 变体兜底命中不校验 dvd_id（跨影片 cid 碰撞即张冠李戴，dmm_api 同类逻辑有打分此处没有）→ 两处补等值校验；⑲ guochan 标签/演员名直接嵌正则（含元字符即 re.error 整链崩溃）→ 子串用 `in`、人名 `re.escape`。新增/修改测试 60+ 项锁定全部修复（v1 迁移/前缀路由/合并策略/共享番号/FC2 匹配/LogBuffer 通道/流式 CF 判定/断点缓存 origin）

- **全库审查第二批 7 项缺陷修复（中等置信度遗留项）**：与第一批同纪律（先复现验证再修，红→绿）。① mosaic 采信顺序非确定——遍历并发完成序的 dict 取首个非空值，同文件重刮可能翻转 mosaic 归类（NFO 标签/文件夹 country 属性抖动）；对齐 thumb_list 的"优先级 sites → 来源排序"双段确定性模式。② `_failed.json` 保护分支把 MARK_FILE 永久指向 `_failed.json`（真校验错误时经 path setter 写标记文件，用户直接关程序后下次启动 reset 写默认值进 `_failed.json` 并永久指向，原配置完好在原处却永不再加载）——新增 `switch_to_failed_session()`（绕过标记写入，`_failed.json` 仅作会话内存配置，MARK_FILE 保持指向原配置，重启即自动恢复），AST 哨兵锁定不得回退为 path 赋值。③ missav `_parse_code_parts` 数字段 `\d{2,6}` 截断 FC2 长 ID（`fc2-ppv-1234567` → `ppv-123456` 错误番号）——放宽至 `{2,8}`（正常 JAV 番号 3-5 位不受影响）。④ Emby/Jellyfin 出演统计分页在 `TotalRecordCount` 缺失/为 0 时 `start_index >= 0` 恒真只拉第一页（大库演员关联数大面积缺失）——字段缺失时退化为仅靠短页信号判定终止。⑤ ASIN xlsx 并发写无互斥——写路径挪 `to_thread` 后并发刮削协程对同一库文件的 load→改→save 会真并行交错丢更新；模块级 `_asin_db_write_lock` 串行化三个写核心与出厂库合并。⑥ 断点缓存 commit 失败无 rollback（`database is locked` 超时后事务悬挂打开，后续写混入旧事务下次一起提交，语义错乱且 flush 再失败即批量状态丢失）——`_execute`/`flush` 失败分支补 rollback 清边界。⑦ `nfo_merger._CRITICAL_FIELDS` 的 number 保护是死代码（合并循环只遍历 `_SCALAR_FIELDS` 不含 number，且上游 file_crawler 总会用文件番号覆盖 `res.number`）——移除 number 消除误导防未来重构误信。新增回归测试 2 项（MARK_FILE 哨兵 + issue69 扩展），相关域测试全绿
