...
- **议题 #73 检测网络页中途异常中断**：检测跑到中途弹出「网络检测出现异常：」（空消息）后整轮停止，后续站点不再检测。根因：`run_network_check` 主循环对单项任务的逃逸异常（绕过底层兜底的深层异常）没有兜底，一项炸掉整轮。修复：单项异常降级为该站点失败项并携带异常类型名（空消息异常不再空白），继续跑完全部站点；异常 traceback 直接展示在检测页，复制结果即可反馈完整诊断。启动自检开头另加范围说明（自检固定四项，全量检测请到「检测网络」页）
