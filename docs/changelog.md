...
- **议题 #76 导航按钮改名**：「主界面」→「软件界面」、「日志」→「软件日志」、「工具」→「软件工具」、「设置」→「软件设置」，使用指南与快速上手文档同步
...
- **新增 7mmtv 聚合站爬虫**（移植自 Hazard804/mdcx）：7mmtv.sx 有码/无码/素人/FC2 剧照与元数据刮削，加入有码/无码/素人/FC2 四个默认源列表；HEYZO 剧照场景适用。双镜像域名轮询（7mmtv.sx → 7tv022.com 失败自动切换），两域名默认加入「走代理网站」列表（老配置需在设置→网络手动补充）；旧配置中残留的 7mmtv 站点值自动恢复有效
